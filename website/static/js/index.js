...
const handle_chats = () => {
    ...
    els.forEach((el, i) => {
        el.addEventListener('click', e => {
            ...
            `
            fetch(`http://localhost:8888/${type}-chat/${name}?offset=0`).then(res => {
                return res.json()
            }).then(res => {
                chats.innerHTML = ''
                // turn off loading
                res.forEach(msg => {
                    ...
                    <div data-id=${_id} id=${_id} class="m_flex_row_start m_align_end m_chat_con ${from === me && 'm_row_rev'}">
                        ...
                        <div class="m_flex_col_cen" ${from !== me ? 'style="align-items: flex-start"' : ''}>
                            ...
                            <img ${from !== me && `onclick='show_info("${from}")'`} alt="user" src="${from === me ? my_profile : `http://localhost:8888/files/get-user-avatar/${avatar}`}" class="profile_pic" />
                        </div>
...
const show_info = (username, type = 'user') => {
    ...
    pp.innerHTML = `
        ...
        <!-- [ldio] generated by https://loading.io/ --></svg>
    `;
    fetch(`http://localhost:8888/${type}-info/${username}`).then(res => res.json()).then(res => {
        if(type === 'user') {
            let {avatar, bio} = res;
            pp.innerHTML = `
                <h1 style="margin: 1rem; color: white; font-size: 30px; font-weight: bold;">${username}'s Profile</h1>
                <div class="m_image_upload_con">
                    <img alt="${username}'s Profile" src="http://localhost:8888/files/get-user-avatar/${avatar}" name="my_own_profile_pic" class="profile_pic m_profile_lg mb-3" />
                </div>
            ...
            pp.innerHTML = `
                ...
                <div class="m_image_upload_con">
                    <img alt="${name}'s Profile" src="http://localhost:8888/files/get-user-avatar/${avatar}" name="my_own_profile_pic" class="profile_pic m_profile_lg mb-3" />
                </div>
                <div class="form-floating m_form_posing mb-3">
